...
// Get returns a value from the SessionStore.
func (s *SessionStore) Get(key interface{}) interface{} {
	return s.session.Values[key]
}

// Set returns a value to the SessionStore.
func (s *SessionStore) Set(key, value interface{}) {
	s.session.Values[key] = value
}

// Delete deletes the SessionStore.
func (s *SessionStore) Delete() {
	s.session.Options.MaxAge = -1
}

// Save saves any changes made to the SessionStore.
