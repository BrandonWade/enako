...
func (s *Stack) ValidateExpense() Middleware {
	return func(f http.HandlerFunc) http.HandlerFunc {
		return func(w http.ResponseWriter, r *http.Request) {
			expense, ok := r.Context().Value(ContextExpenseKey).(models.Expense)
			if !ok {
				s.logger.WithField("method", "middleware.ValidateExpense").Error(helpers.ErrorRetrievingExpense())
			...
			if err := validator.Validate(expense); err != nil {
				s.logger.WithField("method", "middleware.ValidateExpense").Info(err.Error())
